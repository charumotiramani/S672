...
function rotateVector(v, k, rad) {
  ...
  return v.map(
    (vi, i) => vi * cosA + crossKV[i] * sinA + k[i] * kDotV * (1 - cosA)
  );
}
...
function getGainAtAngle(psi, controls) {
  if (controls.gainPatternType.startsWith("Single Feed")) {
    return gainPatternSingleFeed(psi, controls.Gm, controls.psi0, controls.Ls);
  }
  if (controls.gainPatternType.startsWith("MEO")) {
    return gainPatternMEO(
      psi,
      controls.Gm,
      controls.Ls,
      controls.psi_b,
  ...
  if (controls.gainPatternType.startsWith("LEO")) {
    return gainPatternLEO(
      psi,
      controls.Gm,
      controls.Ls,
      controls.psi_b,
...
function computeFootprintElliptical(
  satECEF,
  boresight,
  controls,
  targetGain,
...
function validateContourNesting(footprints) {
  ...
  for (let i = 0; i < sortedFootprints.length - 1; i++) {
    ...
    if (higherGainContour.area_km2 > lowerGainContour.area_km2 * 1.1) {
      // 10% tolerance
...
//         proj = d3.geoAzimuthalEqualArea().rotate(rotation);
//         break;
//       case "stereographic":
//         proj = d3.geoStereographic().rotate(rotation);
//         break;
...
//     legend.innerHTML =
//       `<div style="font-weight: bold; margin-bottom: 5px;">Gain Contours</div>` +
//       sortedFootprints
//         .map((fp, i) =>
//           fp.points.length > 0
//             ? `
...
//               colors[i]
//             }; margin-right: 5px; border: 1px solid #777;"></div>
//             <span>
//               <b>Contour ${fp.index}:</b> ${fp.relativeGain.toFixed(
//                 1
//               )} dB (abs: ${fp.gain.toFixed(1)} dBi)<br>
//               <span style="font-size:11px;">Area: ${fp.area_km2.toLocaleString(
//                 "en-US",
...
function getFootprints(controls) {
  const relativeGains = [controls.gain_rel_1];

  // display(relativeGains);

  const absoluteGains = relativeGains.map((rel_gain) => controls.Gm + rel_gain);
  const satECEF = geodeticToECEF(
    controls.satelliteLat,
    controls.satelliteLon,
    controls.satelliteAlt
  );
  const targetECEF = geodeticToECEF(
    controls.antennaLat,
    controls.antennaLon,
    0
  );
  ...
  let rawFootprints = absoluteGains.map((gain, index) => {
    ...
    var latlng = points.map((p) => [p[1], p[0]]);
    return {
      gain: gain,
